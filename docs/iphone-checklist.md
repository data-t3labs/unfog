...
| A6 | Record indoors / just after launch (coarse first fixes). | Fixes worse than 50 m are dropped — the banner distance stays 0 m and an orange line under it says "GPS accuracy is ±N m — points are skipped until it improves. Try near a window or outside." until accuracy improves; the summary shows "N skipped (poor GPS accuracy)". | |
...
| A16 | Settings › Accessibility › Motion › Reduce Motion on → open Unfog, route somewhere, tap Record, stop it. | The camera jumps instead of easing (MapLibre honours the setting); the red recording dot does not pulse; sheets and toasts appear in place. | |
