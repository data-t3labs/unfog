...
**Wired (2026-09-03).** `RouteEngine` owns a `PackSource` next to `TileSource` and installs it as the tile fallback: memory → prebuilt region → downloaded area → pack cache (IndexedDB `unfog-packs`); the engine, not the fallback, touches the network — `graphFor` fetches the box's locally-missing tiles from their packs first (one coalesced byte-range round per pack, ≤ 4 in flight, 60 s deadline per range) so a first route in a new city costs a handful of requests. `coverage(bbox)` reports `packable` (a pack covers it, not on the device yet); `NoCoverageError` only when `available + packable === 0`, and `route()` answers "a pack covers this but nothing loaded" (offline with nothing cached, a shard not deployed → 404) with the straight-line floor, no prompt — outside every pack the download offer stays; loops keep NoCoverageError. Boot: `packs.init({ refreshTimeoutMs: 5000 })` — a slow packs-index.json never stalls `route.init` (the refresh lands in the background; `cache: 'no-cache'`, 30 s abort). The main thread's `src/app/prefetch-driver.ts` (`startPrefetchDriver(ctx)`, one call in main.ts) feeds the `Prefetcher`: geolocation fixes, `moveend` while no tracking session runs, the map centre at start; a round every 10 s, on `online`, and when the centre tile changes; never offline or on `navigator.connection.saveData`; 150 MB LRU budget sparing the current ring. RouteApi grew `packsHasTile/packsFetchTiles/packsListCached/packsEvict/packsClear/packsStatus` (Comlink; mock = no-ops). Data → Routing data (`src/app/packs-data.ts`): "Automatic: the streets around you download as you go (Wi-Fi and mobile; paused on Low Data Mode)", cached streets grouped by cell as "Streets near New York (US) · 12 MB · used today", Clear, "Coverage list updated N h ago"; Regions and Downloaded areas unchanged beneath it. Service worker: the `graph` CacheFirst rule excludes `/unfog/graph/packs/` (the index must reach the network; a cached full body answered to a Range request would break the 206 contract) — guarded by `tests/unit/sw-runtime-caching.test.ts`; pack bytes never enter a SW cache. Tests: pack-source / tiles-source / williamsburg (engine over packs: coalesced fetch, offline cache, 404 floor, loop NoCoverage, Clear) / prefetch (driver) unit tests; `tests/e2e/packs.spec.ts` serves a fake packs-index + real packs built from public/graph/{nyc,vancouver} through `context.route` on sibling paths (absolute per-cell URLs, Range → 206).
