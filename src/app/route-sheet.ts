...
import { fmtDistance, fmtMinutes } from './format';
...
export function createRouteSheet(ctx: AppContext): RouteSheet {
  const { shell, map } = ctx;
  const prefs = readJSON<Prefs>(ROUTE_PREFS_KEY, { mode: 'walk', detour: 0.25 });
  ...
  const sheet = el('div', { class: 'sheet route', hidden: true }, el('div', { class: 'grab' }), closeBtn, title, modes, sliderRow, slider, cands, status, goBtn);
  ...
  function renderCands(): void {
    ...
    result.candidates.forEach((c, i) => {
      const row = el(
        ...
        el('div', { class: 't' }, el('div', { class: 'name', text: c.name }), el('div', { class: 'st', text: `${km(c.lengthM)} · ${fmtMinutes(c.etaMin)}` })),
  ...
  function fit(): void {
    ...
    const bottom = sheet.offsetHeight + 24;
    map.fitCoords(coords, { top: 150, bottom, left: 40, right: 40 });
  }
  ...
  async function run(): Promise<void> {
    ...
    renderCands();
    map.clearRoutes();
    setStatus(el('div', { class: 'spinner-row' }, el('span', { class: 'spinner' }), 'Finding routes…'));
    origin = await resolveOrigin();
    if (my !== seq) return;
    renderTitle();
    renderSlider();
    const req = { from: origin, to: dest.lonlat, mode: prefs.mode, detour: prefs.detour };
    try {
      const res = await ctx.engines.route.route(req);
      if (my !== seq) return;
      if (!res.candidates.length) throw new Error('No route found between these points');
      result = res;
      selected = 0;
      // Only worth a line when we could not route from the user's position.
      setStatus(originNote.includes('map centre') ? el('div', { class: 'muted small', text: `Routes ${originNote}` }) : null);
      renderTitle();
      renderSlider();
      renderCands();
      ...
      // Route worker: name 'NoCoverageError' (message mentions coverage) → offer a download;
      // 'SnapError' (no road for the mode within 300 m) and anything else → just the message.
      if (err?.name === 'NoCoverageError' || (err?.name !== 'SnapError' && /coverage|no graph|graph data|not covered/i.test(msg))) {
        await offerDownload(origin, dest.lonlat);
      } else {
        setStatus(el('div', { class: 'error', text: msg }));
      }
    }
  }

  async function offerDownload(from: LonLat, to: LonLat): Promise<void> {
    const centre: LonLat = [(from[0] + to[0]) / 2, (from[1] + to[1]) / 2];
    const span = distanceM(from[0], from[1], to[0], to[1]);
    const radiusKm = Math.min(8, Math.max(3, Math.ceil(span / 2000) + 1));
    ...
    const runDownload = async (label: string, task: (onProgress: (p: { phase: string; done: number; total: number }) => void) => Promise<unknown>) => {
      ...
      try {
        ...
        progressText.textContent = `Download failed: ${String((e as Error)?.message ?? e)}`;
  ...
  async function go(): Promise<void> {
    ...
    clear(barText);
    barText.append(el('div', { class: 'name', text: `${c.name} · ${km(c.lengthM)} · ${fmtMinutes(c.etaMin)}` }), el('div', { class: 'st', text: `${Math.round(c.pctNew)}% new · ${dest?.name ?? ''}` }));
    map.showRoutes([c], 0);
    if (ok) map.setFollow(true, 16.5);
    else toast('Following without location — enable it to see yourself on the route');
  }
  ...
  const api: RouteSheet = {
    ...
    open(d) {
      ...
      shell.searchText.textContent = d.name;
